 ...
 *
 * $Id: avi.c 13543 2008-02-03 18:50:03Z khughes $ 
 *
...
int AVI_is_avi(char *name) {
    ...
    movie.streams = (AviStreamRec *) calloc(sizeof (AviStreamRec) * movie.header->Streams, 1);
    ...
    for (temp = 0; temp < movie.header->Streams; temp++) {
        ...
        if (movie.streams[temp].sh.Type == FCC("vids")) {
            if (fcca == FCC("mjpg") || fcca == FCC("MJPG")) {
...
AviError AVI_open_movie(char *name, AviMovie *movie) {
    ...
    for (temp = 0; temp < movie->header->Streams; temp++) {
        ...
        if (movie->streams[temp].sh.Type == FCC("vids")) {
            if (fcca == FCC("mjpg") || fcca == FCC("MJPG")) {
                movie->streams[temp].format = AVI_FORMAT_MJPEG;
            } else if (fcca == FCC("dib ") || fcca == FCC("DIB ")) {
                movie->streams[temp].format = AVI_FORMAT_RGB24;
            } else {
        ...
        if (movie->streams[temp].sh.Type == FCC("vids")) {
            ...
            if (j >= 0) {
                ...
                AviBitmapInfoHeader *sf = malloc(sizeof (AviBitmapInfoHeader));

                movie->streams[temp].sf = sf;
                movie->streams[temp].sf_size = sizeof (AviBitmapInfoHeader);

                sf->fcc = FCC("strf");
                sf->size = movie->streams[temp].sf_size;
                sf->Size = GET_FCC(movie->fp);
                sf->Width = GET_FCC(movie->fp);
                sf->Height = GET_FCC(movie->fp);
                sf->Planes = GET_TCC(movie->fp);
                sf->BitCount = GET_TCC(movie->fp);
                sf->Compression = GET_FCC(movie->fp);
                sf->SizeImage = GET_FCC(movie->fp);
                sf->XPelsPerMeter = GET_FCC(movie->fp);
                sf->YPelsPerMeter = GET_FCC(movie->fp);
                sf->ClrUsed = GET_FCC(movie->fp);
                sf->ClrImportant = GET_FCC(movie->fp);
    ...
    if (movie->header->Flags & AVIF_HASINDEX) {
        ...
        movie->entries = (AviIndexEntry *) malloc(movie->index_entries * sizeof (AviIndexEntry));
...
AviError AVI_open_compress(char *name, AviMovie *movie, int streams, ...) {
    ...
    for (i = 0; i < movie->header->Streams; i++) {
        ...
        if (movie->streams[i].sh.Type == FCC("vids")) {
            ...
            movie->streams[i].sf = sf;
            movie->streams[i].sf_size = sizeof (AviBitmapInfoHeader);

            sf->fcc = FCC("strf");
            sf->size = movie->streams[i].sf_size - 8;
            sf->Size = movie->streams[i].sf_size - 8;
            sf->Width = 0;
            sf->Height = 0;
            sf->Planes = 1;
            sf->BitCount = 24;
            sf->Compression = avi_get_format_compression(movie->streams[i].format);
            sf->SizeImage = 0;
            sf->XPelsPerMeter = 0;
            sf->YPelsPerMeter = 0;
            sf->ClrUsed = 0;
...
AviError AVI_write_frame(AviMovie *movie, int frame_num, ...) {
    ...
    if (frame_num + 1 > movie->index_entries) {
        temp = (AviIndexEntry *) malloc((frame_num + 1) *
                (movie->header->Streams + 1) * sizeof (AviIndexEntry));
        if (movie->entries != NULL) {
