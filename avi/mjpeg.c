 ...
 *
 * $Id: mjpeg.c 12931 2007-12-17 18:20:48Z theeth $
 *
...
static void add_huff_table(j_decompress_ptr dinfo, JHUFF_TBL **htblptr,
        const UINT8 *bits, const int bits_len, const UINT8 *val, const int val_len) {

    if (*htblptr == NULL)
        *htblptr = jpeg_alloc_huff_table((j_common_ptr) dinfo);

    memset((*htblptr)->bits, 0, sizeof ((*htblptr)->bits));
    memcpy((*htblptr)->bits, bits, bits_len);

    memset((*htblptr)->huffval, 0, sizeof ((*htblptr)->huffval));
    memcpy((*htblptr)->huffval, val, val_len);
...
static void std_huff_tables(j_decompress_ptr dinfo) {
    static const UINT8 bits_dc_luminance[] = {/* 0-base */
    ...
    static const UINT8 bits_dc_chrominance[] = {/* 0-base */
    ...
    static const UINT8 bits_ac_luminance[] = {/* 0-base */
    ...
    static const UINT8 bits_ac_chrominance[] = {/* 0-base */
    ...
    add_huff_table(dinfo, &dinfo->dc_huff_tbl_ptrs[0],
            bits_dc_luminance, sizeof (bits_dc_luminance),
            val_dc_luminance, sizeof (val_dc_luminance));
    add_huff_table(dinfo, &dinfo->ac_huff_tbl_ptrs[0],
            bits_ac_luminance, sizeof (bits_ac_luminance),
            val_ac_luminance, sizeof (val_ac_luminance));
    add_huff_table(dinfo, &dinfo->dc_huff_tbl_ptrs[1],
            bits_dc_chrominance, sizeof (bits_dc_chrominance),
            val_dc_chrominance, sizeof (val_dc_chrominance));
    add_huff_table(dinfo, &dinfo->ac_huff_tbl_ptrs[1],
            bits_ac_chrominance, sizeof (bits_ac_chrominance),
            val_ac_chrominance, sizeof (val_ac_chrominance));
}
...
static int check_and_decode_jpeg(unsigned char *inbuf, unsigned char *outbuf, int width, int height, int bufsize) {
    ...
    if ((width & 0xF) || (height & 0xF)) {
        ...
        unsigned char *tmpbuf = malloc(jwidth * jheight * 3);
...
void *avi_converter_from_mjpeg(AviMovie *movie, int stream, unsigned char *buffer, int *size) {
    ...
    buf = malloc(movie->header->Height * movie->header->Width * 3);
    ...
    if (deint) {
        buffer = malloc(movie->header->Height * movie->header->Width * 3);
...
static void jpegmemdestmgr_build(j_compress_ptr cinfo, unsigned char *buffer, int bufsize) {
    cinfo->dest = malloc(sizeof (*(cinfo->dest)));
...
static void jpegmemsrcmgr_build(j_decompress_ptr dinfo, unsigned char *buffer, int bufsize) {
    dinfo->src = malloc(sizeof (*(dinfo->src)));
